...
쌍으로 기다리지 않고 파일명 기준으로 각각 처리한다):
  ...
  Telecom_Benefit_Info_DB.csv      → benefit_info (동일 스키마, telecom_data/ 크롤러 전용 파일명)
...
TARGETS = {
    ...
    # headless/클라우드 IP 차단 때문에 Cloud Run Job이 아니라 별도 Xvfb+headed GCE
    # VM에서 도는 스크래퍼(galaxy_store/zeropin/gmarket/naver_brandstore/
    # lgu_event_board/cultureland_cash_conversion/samsung_pay)의 업로드 파일명.
    # 같은 도메인의 Cloud Run Job 업로드와 GCS 목적지가 겹치면 서로 덮어쓸 수 있어
    # 파일명을 분리했다(전부 동일 스키마, benefit_info로 적재).
    ...
    "Epay_Benefit_Info_DB_vm.csv": (clean_benefit_info_rows, load_benefit_info),
}
