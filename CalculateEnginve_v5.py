...
import itertools #itertools는 파이썬에서 반복 가능한 객체를 생성하고 조합, 순열 등을 다루는 모듈이다. 이 코드에서는 generate_combinations 함수에서 itertools.product를 사용하여 결제 계층별로 '적용/미적용' 모든 경우의 수를 생성하는 데 사용된다.
