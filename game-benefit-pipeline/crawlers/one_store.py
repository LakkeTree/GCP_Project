...
class OneStoreCrawler(BaseCrawler):
    ...
    context_hint = (
        ...
        "1차/2차 쿠폰이 같이 적힌 경우 두 개의 별도 항목으로 분리하세요. ★ 이때 "
        "item_or_event_name에는 반드시 '[1차]' 또는 '[2차]' 표시를 그대로 남겨서, "
        "같은 게임의 두 항목이 이름만으로도 서로 구분되게 하세요. 표시를 빼면 "
        "두 항목이 같은 혜택으로 잘못 합쳐질 수 있습니다.\n"
