...
class GalaxyStoreCrawler(BaseCrawler):
    ...
    context_hint = (
        ...
        "8,000원 쿠폰을 받는 구조), 이건 게임 하나짜리 혜택이 아니라 "
        "게임 개수만큼의 서로 다른 혜택입니다. 목록에 있는 게임 하나하나를 "
        "별도 항목으로 분리하고, 각 항목의 target_game에 그 게임의 이름을 "
        "정확히 넣으세요. target_game을 전부 ALL로 두거나 item_or_event_name을 "
        "게시글 제목 그대로 복사하면, 서로 다른 게임의 혜택인데도 시스템이 "
        "같은 혜택으로 착각해서 하나만 남기고 나머지를 지워버립니다.\n"
    ...
    # ⚠️ Gemini 무료 등급은 모델당 하루 호출 수가 매우 적습니다(예: 20건).
    # 15개씩 열면 이 크롤러 하나가 하루 할당량을 혼자 다 써버려서 원스토어・
    # 구글플레이가 아예 못 돌 수 있습니다. 낮게 잡아두고, 유료 등급으로
    # 전환하시면 다시 늘리셔도 됩니다.
    MAX_DETAIL_PAGES = 5
